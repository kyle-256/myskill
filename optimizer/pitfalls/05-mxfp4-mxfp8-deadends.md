...
## ★★★★ mxfp4 dense 深-K:2026-09-14 一次性关掉的六条轴(全部实测,别重踩)

背景:MXFP4 dense GEMM(gfx950,256×256×256,4-wave,LDS 147456,vgpr 488+agpr 256),
目标是每一行都超过 AITER。欠账行 FC1_wgrad(28672×4096×32768)与
out_proj_wgrad(4096×4096×32768),两者都是 `tiles % 256CU == 0` 的均衡网格。

| 轴 | 判定 | 实测 | 根因 |
|---|---|---|---|
| **强制 split-K / tail-split**(绕开 `_ksplit_candidates` 的 `eff>0.95 return [1]`) | ❌实测DEAD | out_proj_wgrad −9.6/−24.9/−45.9%(ks 2/4/8);FC1_wgrad −9.5/−25.0/−44.7%,tail −2.6/−5.3% | 均衡网格上 split 只多搬 partial 字节(uniform split ksplit=2 就要多搬 (s−1)·M·N·2 ≈ 4.2% 的流量预算),换不回延迟隐藏。**那个筛选器是对的** |
| **同步密度 n=3 / n=4**(对齐 AITER 反汇编的 4 barrier) | ❌实测DEAD | 全表 −0.050% / −0.191%,5~7 行回退 | ★**我们的 barrier 已经是 6、AITER 才 4** —— 不是同步不够,是早就过头了。见 methodology/03 §ISA dump |
| **不对称同步**(phase_a 与 inplace 各自密度) | ❌实测DEAD | (2,0) −0.415% / (0,2) −0.007% / (1,3) −0.124% | 对称 both=2 已在最优点 |
| **深化 vmcnt 水位**(`_wl_opts` 上界 16 → 22 / 28) | ❌实测DEAD | 全表 −0.357% / −0.099%;**FC1_wgrad 完全无反应**(−0.072/−0.017%) | ISA 显示深-K 已选中更深的 (16,15);再深无收益 |
| **autotune 缓存键补 `prepacked` 维** | ❌实测DEAD | **9/12 → 7/12**;QKV_wgrad 1.0150→0.9595、FC2_wgrad 1.0117→0.9878 | prepacked 对着真臂自己调,反而比蹭重排 build 的 config 差。冷卡调优的解释**已被实验排除**(把 150s 预热挪到 autotune 之前,0.9595→0.9618,仍 7/12)。⚠ 与 pitfalls/10 §cache key 那条**不是同一件事**:那条影响 build 语义(必修),这条只影响调优排名(必须实测) |
| **barrier 按 MFMA 计数均衡**(而非按行数) | ❌实测DEAD | 三跑 +0.012% / −0.322% / +0.050%;4 行稳定回退、目标两行全变号 | 机制真实(旧切法每段 MFMA 12/42/74,新切法 43/43/42)**但净零**。见 pitfalls/02 §持平也要重复 |

★★★★★ **这六条全白关** —— 缺口是分母造出来的,见 **methodology/20**。
FC1_wgrad 对真 AITER 头对头其实 **+2.148%**。

### 顺带:这个核的循环画像(可作其它 mxfp4 核的对照基线)
稳态 K 循环 **381 条指令 / 256 MFMA(`v_mfma_scale_f32_16x16x128_f8f6f4`)/ 64 ds_read_b128 /
36 buffer_load_dwordx4 / 2 s_waitcnt(`vmcnt(16) lgkmcnt(15)`)/ 6 s_barrier / 15 s_ALU /
2 s_nop / v_ALU 0 / ds_write 0**。67% 指令是 MFMA,LDS→VGPR 复用已满
