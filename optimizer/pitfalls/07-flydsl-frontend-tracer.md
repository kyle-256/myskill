...
## ★★★★★ traced kernel 里参与【边界/寻址】的量必须来自 traced 值(2026-09-15)

给 FlyDSL quant 的 scale store 加了一条打包布局分支,SRD 的 `num_records` 写成宿主字面量:

```python
rscrsrc = _srd(ROW_SC, grsc, 1, pack_row["qm"] * pack_row["k128"] * 4)   # 全是 Python int
```

四个形状依次跑,**第四个只写了前 524288 字节、高行全零** —— 而 524288 正是**第一个形状**的
slab 大小。编译缓存没按这个常量区分 kernel,第四个形状复用了第一个的 bound。

★★★★★ **前两个形状「通过」是运气**:它们先跑的那个 slab 更大,截断没发生。
⇒ **一个 bound 相关的 bug,在「先大后小」的顺序下会完全隐身**。改成从 traced 的 `R`/`C` 推导:

```python
_qm = ((R + 255) >> 8) << 8            # R 是 fx.Int32
rscrsrc = _srd(ROW_SC, grsc, 1, _qm * ((C >> 7) * 4))
```

⇒ 规矩:**凡是 `num_records` / SRD extent / 掩码上界,一律从 kernel 的 traced 参数推出来**,
哪怕你「知道」它是常数。宿主常量只允许出现在**改变代码结构**的地方(分支、循环次数、向量宽度),
那些本来就会改变 IR。

★ 配套:`_srd` 之类的本地 helper 要能接受宿主字面量(`arith.index(v) if isinstance(v, int)
else arith.index_cast(...)`),否则传 0 都会炸 `'int' object has no attribute '_CAPIPtr'`。

## ★★★★ tile 化 kernel 的「全局坐标」不一定是你看到的那个变量(2026-09-15)

同一个 quant 里:

```python
_row0 = fx.Int32(0) if _fold else r0        # ← _fold 时 grow 是 TILE 内相对行
grow  = _row0 + r_row
sc    = grow * (cpad >> 5) + gcmb           # 配套的 SRD 已按 tile 重基,所以相对行是对的
```

我要写一个**按全局坐标寻址**的新布局,直接拿 `grow` 去算 ⇒ 所有 tile 塌到同一段偏移,
**只写了 1/64 的输出**。正确写法是 `r0 + r_row`(两种 `_fold` 下都对)。

⇒ 动一个 tile 化 kernel 的 store 地址前,**先确认那个「看起来像全局行」的变量是不是被
SRD 的 per-tile rebase 抵消掉了**。判据:看它配套的 `_srd(...)` 有没有乘 `r0`/`c0`。

## ★★★★★ 宿主常量参与【寻址】比参与【边界】更隐蔽(2026-09-15,同日第二次)

上一条说的是 `num_records`。同一天又踩了它的孪生兄弟:地址算式里的 `k128`。

```python
mxfp4_packed_scale_byte(col, kblk, k128=pack_col["k128"], ...)   # 宿主 int
#   内部:kk = k128 // n_sub,直接进地址表达式
```

⇒ **一个 K 一个编译产物**,跨形状复用时拿到别人的常量。症状:部分输出从未被写,
**缺失跨某一维完全均匀**,读起来像「地址撞车」而不是「边界截断」。

排查它花了很久,因为它同时**看起来像形状固有**(同两个形状总是失败)**又像缓存问题**
(单独跑就对)。★ **决定性实验 = 把形状倒序再跑一遍**:
- 失败跟着形状走 ⇒ 形状固有
- 失败跟着位置走 ⇒ 顺序/缓存
- **两种顺序下同两个形状都失败、且倒序时它们跑在最前面** ⇒ 两者都不是,还有第三个变量

修法:把**唯一随收缩变化的项**作为 traced 值传进去(`kk=R>>8`),`ku` 这种真正改变代码结构的
留作编译期。于是**一个 kernel 服务所有形状,复用在构造上就是对的**,而不是靠缓存键碰巧不撞。

⇒ 规矩(合并上一条):**随形状变化的量进 traced kernel,要么是 traced 值,要么确认它真的改变 IR。**
夹在中间的「宿主常量参与寻址/边界」最危险 —— 它**只在特定的形状顺序下暴露**,
先大后小、或单形状测试,都会给出「通过」。
