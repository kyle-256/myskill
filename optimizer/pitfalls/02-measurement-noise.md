...
## ★★★★★ 噪声的第四个来源:同一份源码编出的 kernel 就不一样(2026-09-14 gfx950 实测)

一直把 bench 的 run-to-run 散布归因为计时/DVFS/邻居。实测还有一条**编译期**的:

```
FLYDSL_DUMP_IR=1 FLYDSL_RUNTIME_ENABLE_CACHE=0 → 21_final_isa.s
同一棵树,连编两次:
  kern_0 (preshuffle)        e01c02df  =  e01c02df     稳定
  launch_mxfp8_fused (GEMM)  2c146346  ≠  d031b941     不稳定
```

指令数只差 6(7333 vs 7339)、VGPR/LDS 相同,但寄存器分配被整体 renumber。根因是
**FlyDSL 进程内 autotune**(磁盘 autotune 目录只有空 `.lock`)——每次跑都重新选一次 config,
选中哪个带随机性,于是**每次 bench 量的可能是一个不同的 kernel**。

三条推论:

1. **单样本 A/B 对 <1% 的效应无效。** 同一份代码两遍读到 106.3974 与 107.1200,差 0.72 点,
   比要判的效应还大。
2. **ISA md5 只能对「同树两编 md5 相同」的那些 kernel 当等价判据**,用之前必须先做这个对照
   (见 methodology/16 §先找确定性判据)。
3. 想让 <1% 可判,只能靠**每臂 5+ 样本 + 交替臂序**,或者换成确定性判据;否则正确答案是
   「测不出来」而不是一个数。
...
## ★★★★★ 自写 A/B 探针必须内建 A-vs-A 校准臂(2026-09-14,抓出一个 +54% 的假象)

给 MXFP4 两行慢形状强制各 launch mode 定价,第一版探针读出:

```
FC1_wgrad mode (2,2): gain +53.919%     # 一个已跑到对手 98.9% 的 kernel
out_proj_wgrad (1,8): gain +29.414%     # 换个启动模式快一半?
```
...
for _ in range(QUARTETS):
    GK._MXFP4_KSPLIT_CACHE[key] = (0,1); GK._MXFP4_LAUNCH_CACHE.clear()   # 清
    p1 = t_one(plain)
    GK._MXFP4_KSPLIT_CACHE[key] = m;     GK._MXFP4_LAUNCH_CACHE.clear()   # 清
    s1 = t_one(cand); s2 = t_one(cand)                                    # s2 没清
    GK._MXFP4_KSPLIT_CACHE[key] = (0,1); GK._MXFP4_LAUNCH_CACHE.clear()   # 清
    p2 = t_one(plain)
    ratios.append(((p1+p2)/2) / ((s1+s2)/2))
```

⇒ plain 臂每四元组付 **2 次**清缓存后的首发查找,候选臂只付 **1 次**。
**系统性地把基线臂拖慢**,比值虚高。

**修法两条**:
1. 计时段内**绝不清编译/启动缓存**;换配置后先跑一发**不计时**的热身:
   ```python
   def set_mode(key, m, fn):
       GK._MXFP4_KSPLIT_CACHE[key] = m
       fn(); torch.cuda.synchronize()      # untimed
   ```
2. ★**把基线当候选再跑一遍同样的四元组**,读数必须 ≈0:
   ```python
   for m in [BASELINE] + candidates:      # BASELINE 对自己 = A-vs-A 校准
   ```
   修完校准读 **+0.279% / +0.140%**,候选全变负(−2.6% ~ −45.9%)—— 结论完全反号。

⇒ **没有校准臂的自写 A/B 不能判生死。** 校准臂几乎不花钱(多一个候选的时间),
却是唯一能抓住「两臂待遇不对称」这类偏置的手段 —— 噪声地板、ABBA、重复跑**都抓不到它**,
因为偏置是确定性的、每次都朝同一个方向。

同期第二例:同进程头对头对标 AITER,12 行各带一条校准臂,全部落在 −0.564%~+0.186%,
最小余量 +2.148% 才敢报。见 methodology/20。

## ★★★★★ 改「全局常量」前必须全表定价(2026-09-14,与 TPW_MAX 同坑第二次)

`_MXFP4_MID_SYNC`(热循环中插 barrier 的段数)是个**全局**常量。
盯着两行欠账形状调它:

| n | out_proj_wgrad | FC1_wgrad |
|---|---|---|
| 3 | **+0.83%** | +0.12% |
| 4 | **+0.85%** | +0.16% |
...
而且多行在 n=3↔n=4 之间**变号**(QKV_dgrad +0.009→−0.763、FC2_dgrad −0.604→+0.360),
极差 1~2% ⇒ 那两行的初读本来就是噪声。

第一次踩是 `TPW_MAX`:深-K persistent 循环在 4 行子集上两次读到 +0.26%,
全表 **−1.2%**,因为同一个全局值把 FC1_fwd 从 1.09 压到 1.0147。

⇒ **只看目标行调全局常量,必然选出全局净负的值**(目标行是你挑出来的极端点,
不代表分布)。判据:**全局量 ⇒ 全表定价,无例外**;只有 per-shape 可分派的量才允许
只看目标行。

## ★★★★ 「持平」的结论同样需要重复(2026-09-14)

把 barrier 从「按行数均分」改成「按 MFMA 计数均分」(机制真实:离线验过旧切法在
load 前置的相位上给出每段 MFMA **12/42/74**,新切法 43/43/42)。

单跑 geomean **+0.012%**,而且**亏的全是余量 2~11% 的行、赚的全是卡在 1.0 的行** ——
对「所有行超过对手」这个目标看起来正是想要的交换,差点留下。

跑满三次拆开看:

- **三跑同号的只有 5 行**:4 个**稳定回退**(−0.19% ~ −0.50%),1 个稳定增益(+0.65%)
- **目标那两行全变号**:`+0.342/−0.030/+0.471`、`+0.217/−0.001/+0.219` ⇒ 尺子分辨不出

⇒ 净零 = 「4 个稳定亏 + 7 行纯噪声」,不是「无害可留」。
**只有正收益才需要重复验证** 是错的;**geomean≈0 的改动更需要**,因为单跑的符号
完全由噪声决定,而你会不自觉地用「它帮了我关心的行」去合理化它。

## ★★★★★ 配对尺子的第二类偏置:**两臂的 autotune 开销不同**(2026-09-14)

同进程配对 A/B(两个模块实例 + ABBA)能消掉漂移,但消不掉**两臂工作量不对称**。
实例:给候选臂把 `_wl_opts` 从 2 个候选改成 1 个(想定价 wl 深度),读出:
...
★ 蹊跷点:**强制 (10,9) 本来就是调优器平时的选择**,应该读 ≈0,却也读 −0.22%。
⇒ 差异不来自「选中哪个配置」,来自「候选表少一半 ⇒ 调优时长不同 ⇒ 两臂开跑时卡的热度不同」。

受控验证(同一天、同一台机):

| 测试 | geomean |
|---|---|
| **A-vs-A(两臂逐字节相同)** | **−0.098%** |
| **同一批赢家 + 多一次调优试跑**(只改开销,不改结果) | **−0.144%** |

⇒ 只改调优开销就能造出 −0.14%,而两个「强制」臂离地板只有 0.1~0.15% ⇒ **wl 轴无可测效应**。

**规矩**:配对 A/B 的两臂必须 **(a) 源码只差你要定价的那一处,且 (b) autotune 的候选数量相同**。
要定价「候选表放开」这类改动,别用「删候选」当对照 —— 改成**加一个与现有赢家重复的候选**
(赢家集合不变、开销变),先把这个开销项本身定价出来再说。

## ★★★★ 尺子地板会随机器占用漂移,每次都要重标(2026-09-14)

同一把配对尺子、同一份代码:

| 机器状态 | A-vs-A geomean | 单行最负 |
|---|---|---|
| 全空(8 卡 0.6 GiB) | **−0.009%** | — |
| 有邻居(GPU0 13.0 / GPU6 40.5 GiB) | **−0.098%** | **−0.893%** |

⇒ 有邻居时**低于 0.3% 的效应不可分辨**,继续做微调实验只会产生噪声结论。
**每一轮 A/B 都跑一条 A-vs-A 臂当天标定**,别用上次的地板。
同日另一例:绝对口径 bench 因为邻居回来,mxfp4 从 0.996 掉到 0.9824(−1.2%),
差点把一个**已证逐位 no-op** 的改动判成回退 ⇒ **绝对口径 bench 必须同时记机器占用快照**。
