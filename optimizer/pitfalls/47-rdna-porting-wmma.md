...
- **硬件 stochastic-round FP8 转换（`V_CVT_SR_*`）须由 kernel 用 per-lane PRNG（Philox/LCG/threefry）驱动 seed，并每次调用推进它**，否则 'stochastic' rounding 退化成有偏的 deterministic rounding。
  - gfx950 上 `V_PRNG_B32` 用一条 VALU op 推进 per-lane LFSR；multipass SR converters 只**读**不**写** PRNG VGPR，所以必须自己推进。
