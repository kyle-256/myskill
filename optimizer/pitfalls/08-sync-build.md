...
- 验证：`import primus_turbo.pytorch; importlib.import_module("primus_turbo.pytorch._C")` 后 `torch.ops.primus_turbo_cpp_extension.quantize_fp8_tensorwise(x[64,2880]bf16, float8_e4m3fn, None, 128)` → `(64,2944)`，同一调用末位传 `1` → `(64,2880)`。见 [[project_syncv3_so_rebuild_4arg]]、methodology/17 变体 B。
- ★**复发条件不止"迁机器"，`cp -a` 建新环境同样中招**（2026-08-13 syncv4/smci355 二次踩）：syncvN 环境是照 gpt_oss/04 卡 `cp -a` 老 repo 起的，**连旧 `.so` 一起拷过来**；kernel-only bench 不碰 cpp quant，所以能一直绿着，直到第一次跑 e2e 才炸。**新建 syncvN 环境后、首次 e2e 前，先跑上面那条验证**（两秒的事），别等训练崩。
- ⚠️**编译时长按机器差 3 倍，别照抄 7min 设超时**：Crusoe 236 核记录 ~6–7min；**smci355 256 核 / MAX_JOBS=96 实测 ~18min**（phase1 kernels lib 的 `-fgpu-rdc --hip-link` LTO 占大头，期间只刷 `lld: warning: loop not unrolled` 且 `.so` mtime 一直不动，**别误判成卡死**）。判完成看 `BUILD_RC=0` + `.so` mtime 变新，不看日志停没停。
