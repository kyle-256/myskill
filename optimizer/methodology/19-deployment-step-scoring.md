...
## ★★★★★ 榜单的 `tok/s/chip` 有 **99% 是输入 token** ⇒ 解码优化该比的是 `Median ITL`(2026-09-14)

`rocm-llm-bench` 的 `results.csv`(GLM-5.2 / TP4-EP4 / c8)逐列拆开:
```
Token Throughput per Chip = 11261.70  =  Input 11175.17 + Output 86.53
                                                   输出只占 0.77%
P90 Interactivity = 108.11   而   1 / Median_ITL = 1/0.00652 = 153.4
```
⇒ **y 轴(tok/s/chip)基本是 prefill 吞吐**,解码侧的改动在上面几乎看不见。
⇒ **`P90 Interactivity` 不是纯解码率**,它含 TTFT 摊销,系统性低于 `1/ITL`。
⇒ 想把自建解码测量和官方表对账,**唯一同口径的是 `Median ITL`**:
   `1/ITL × 并发` = 该配置的总解码 tok/s。

实测对账(我方合成负载 vs 官方 `_base`,单位 tok/s):
| 并发 | 官方 `1/ITL × 并发` | 自测 base | 比值 |
|---|---|---|---|
| c8 | 1227 | 1140.8 | 0.93 |
| c10 | 1372 | 1214.1 | 0.89 |
| c14 | 1540 | 1437.5 | 0.93 |
**三点一致偏低 7-11%(方向一致而非随机散开)**,机制:合成负载无前缀复用,
而 AgentX 回放开 hierarchical cache ⇒ 平均解码上下文更短。
⇒ 「一致的偏差 + 能说清的机制」比「差不多」强得多,是对账该追求的形态。

★ **别拿别人 `_ours` vs `_base` 的差值给自己的改动定预期**:那两臂
`server_args` 逐字段比对 **14806 字符里只有 `random_seed` 不同** ⇒ 差异全在代码里,
而日志没记录树版本 ⇒ **根本不知道它测的是哪个改动**。它的作用域是「那次改动」。

## ★★★★★ 含 prefill 的总吞吐**不能**用来判解码优化的 A/B(2026-09-13 实测)

同一对代码、同一个负载,只换窗口长度:
```
c10  总吞吐(tok_per_s)   600s: +0.00%   3600s: +19.99%
c10  纯解码(log median)  600s: +16.88%  3600s: +16.91%
```
c14 B 臂的总吞吐从 600s 的 1420.73 掉到 3600s 的 **691.21**,而同一读的纯解码只差 0.36%。
根因查实:该点 `Prefill batch` **2642 行** vs `Decode batch` **1338 行**
——**这个量测的主要是 prefill 占比**,不是被改动的东西。同代码两读散布 6.66%,比效应还大。

⇒ 解码侧的 e2e A/B,主指标取 **server 日志 `gen throughput` 的中位**(只统计解码批次)。
   实测同代码两读散布 **0.14%**,增益跨 600s↔3600s 复现到 **0.03-0.23 个百分点**。
⇒ **先花一个点测同臂两读的噪声底,再决定用哪个指标**。这次正是这一读把主指标从
   「总吞吐」换成了「纯解码」——否则会拿一把噪声 6.66% 的尺子去判 8% 的效应。
⇒ 报的时候必须写明这是**解码侧上界**:真实负载里还有 prefill,合成负载又没有前缀复用,
   两种偏差方向相反。
