...
## ★★★★ 收官验收面必须大于计分面(2026-09-03,dense fp8 GEMM 实测)

campaign 的 goal 里写了「winner 形状不计分,但不许拖慢」(RL6),我收官时只跑了 5 个 P0——
**结果交付版在部署表的兄弟形状和 llama 上都有回退,P0 一格都看不出来。**

正确的收官验收 = 三层,缺一层就可能 ship 出回退:

1. **计分面**(本例 5 个 P0):确认分数没被收尾改动吃掉。
2. **部署全表**(本例 18 个形状 × 三布局,按部署时间占比加权):计分面只占 39.2%,另外
   60.8% 从没被 campaign 量过。
3. **兄弟模型**(本例 llama-2 7B/70B × m∈{4096,8192} × 三布局 = 48 格):同一个 kernel 服务的
   其它模型。**14% 的回退就是在这一层才暴露的**(见 pitfalls/06 race 那条)。

每一层都用**同会话交替配对 A/B**(A=base、B=候选 交替跑,取配对 Δ 中位),不要跨会话比绝对值。

★ 为什么计分面必然盖不住:campaign 的 P0 是按「部署时间占比」挑的少数几个形状,它们在
autotune / dispatch 里往往落在**同一个分支**。本例 5 个 P0 里只有 1 个 TN 形状,而 TN dispatch
恰好是被改坏的那条路径——**一个形状代表不了一条 layout 分支的可达配置集**。
...
## ★★★★★ 收官第四层:逐组件消融定价 —— 「这 1200 行都是必须的吗」

计分面 / 部署全表 / 兄弟模型三层回答的是「**交付会不会把别的东西弄坏**」。它们回答不了另一个
问题:**交付里每一块代码自己挣了多少钱**。

campaign 是一轮一轮往上堆的:某一轮的候选是三四个机制打包过的门槛,谁贡献了多少从没拆开过;
replan 之后换方向,旧机制留在盘上没人再动它。**结果就是交付里混着一批从没单独定过价的搭车货。**
两场 mxfp8 campaign 交付 1121 行,实测下来其中 55 行是零收益、159 行单位代价最差。

### 方法:一次关一个,前后各夹一次 BASE

```
BASE_pre → 关掉组件1 → 关掉组件2 → ... → 关掉组件N → BASE_post
```

每臂 = 一次完整计分。**BASE 必须前后各一次**:它们之间的差就是这段时间的漂移,也就是这张表的
**噪声底**。没有这个底,所有小数字都不可解读。

前提是每个组件都能被**一个常量**关掉(`_MX_BPERM = 1 → 0`、`_QD_KRUN = 3 → 1`、把 split-K 的
`_MX_SPLIT_MIN_KI` 抬到 999999)。这反过来是对写代码的要求:**每个可选机制都应该有一个单点
开关和一个 no-op 值**,否则收官时它无法被定价,只能靠嘴说「应该有用」。

### ★ 验收标准(判去留就看这一条)

> **关掉它的代价 < BASE-to-BASE 漂移 ⇒ 它没在给自己挣钱 ⇒ 删。**

配套两条辅助判据:

- **逐格符号**:真信号在五格上同号(哪怕幅度不同)。**有的格正有的格负 = 噪声的形状**。
  实测 `_QD_KRUN` 关掉后 QKV +0.0 / linear1 +0.3 / MLP-down −0.7 —— 一半格子是正的,判死。
- **单位代价**:收益 ÷ 行数。同样值 0.6%,50 行的留、159 行的重新设计。

### 实测样张(2026-09-14,mxfp8 两场交付,底噪 0.43%)

| 关掉谁 | 代价 | 行数 | 判定 |
|---|---|---|---|
| scale 打包 | −7.60% | 贯穿 | 留 |
| tail-round split-K + fold | −2.27% | ~150 | 留(**最差格跌破 100 硬门**) |
| B 列配对 | −1.51% | ~50 | 留(单位代价最好) |
| epilogue 象限错峰 | −0.96% | ~30 | 留 |
| quant 2d_flat | −0.96% | ~40 | 留 |
| quant sec_pair | −0.84% | ~60 | 留 |
| preshuffle 加宽 | −0.58% | ~159 | **边缘**:只比底噪高 0.15pp,却是最大单块 |
| quant 长 run 走位 krun | −0.39% | ~35 | **删**(低于底噪 + 逐格符号不一致) |
| wave_id 重映射 | **+0.00%** | ~20 | **删**(关掉后五格里四格还微升) |

★ `split-K` 这条值得单记:静态分析显示它在 15 次 GEMM 调用里**只开 4 次**,占比 69.4% 的
linear1/linear2 **一次都不开** —— 看起来像该删的死码。实测却是 −2.27% 且最差格跌到 99.66
**破硬门**,因为它精准压在最弱的两格上。**「覆盖率低」不是删除理由,覆盖到哪里才是。**

### ★★★★★ 先找确定性判据,而且判据自己要先做 A-vs-A 对照

消融是在**噪声尺子**上比大小。凡是能改成**确定性判据**的,不要用尺子:

> **重构声称「等价」⇒ 比编译产物,不比时间。** `FLYDSL_DUMP_IR=1` + `FLYDSL_RUNTIME_ENABLE_CACHE=0`
> dump 出 `21_final_isa.s`,两棵树 md5 相同 = 机器码一致 = 不可能有性能差,**不需要 bench**。

2026-09-14 实测:把 161 行 fork 的 preshuffle 折回共享 helper,ISA md5 `e01c02df` 前后相同,
5 分钟结案。而在此之前我用 bench 烧了一个多小时,还得出了两个互相矛盾的错误结论。

★★★ **但判据自己必须先过 A-vs-A:同一棵树编两次,md5 相同才能用它判等价。** 同一次实测里:

| kernel | 同树两次编译 | 能否当判据 |
|---|---|---|
| `kern_0`(preshuffle) | `e01c02df` = `e01c02df`,四次全同 | ✅ 能 |
| `launch_mxfp8_fused`(NT GEMM) | `2c146346` ≠ `d031b941` | ❌ **不能** |

GEMM 那个核**同一份源码两次编译产物就不同**(编译期 autotune 选 config 带随机性)。我差点据此
得出「helper 合并改变了 GEMM 码生成」的第四个错误结论 —— **对照拦住了它,而对照本该是第一步。**

⇒ 推论一条,比这次重构本身更值钱:**bench 的单样本方差里含「这次编出了一个不同的 kernel」,
不只是计时噪声。** 所以 <1% 的组件根本不可能用少量样本判定,消融表里 0.4~1.0% 那几档要么补到
每臂 5+ 样本交替臂序,要么就别删。

### ★★★★★ 禁止挑样本

同一份代码同一配置跑出来的点,**没有独立理由就不许剔除**。踩证(同一场):四个 NEW 样本
106.3974 / 106.5862 / 107.0579 / 107.1200,我把最低那个当"离群点"扔掉,中位从 106.82 变成
107.06,结论从 −0.93% 变成 −0.71% —— **数字是我挑出来的,不是测出来的**,被用户当场抓住。

配套:**两臂的样本数必须对称**。给 A 测 2 次、给 B 测 1 次然后比中位,这个对比没有效力;我当时
还据此连下三次结论,一次比一次肯定。臂内自差 > 跨臂差时,唯一正确的话是「测不出来」。

### 实施注意

1. **sed 先干跑**:每条替换先 `grep -c` 确认命中且只命中 1 行。改没生效的一臂会读成「这组件
   没用」,是最难发现的假阴性。
2. **每臂结束 `git checkout -- <file>` 还原**,下一臂从干净树出发;跑完核对 `git status` 干净。
3. **臂序固定时末槽占便宜**(本库实测 0.8–1.1pp),所以 BASE 要夹两头;想更严就 A-B-A 逐组件做,
   但那是 3N 次 bench,通常不值。
4. 每臂一次 bench 就够 —— 判据是「代价 vs 底噪」,不是「这一臂的绝对值准不准」。
5. 长跑挂 cron 盯,别 `sleep` 蹲守(见 feedback_longrun_cron_not_sleep)。

驱动脚本与报告器模板:`sync/flydsl_optimizer/_ablate_mxfp8.sh` + `_ablate_report.py`。
