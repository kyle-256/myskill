...
- **venv 按分支选**:
  ...
  - 生产 mxfp4/mxfp8 → `/opt/venv`(容器内默认 venv)
  - 完整示例(tensorwise):`./.ssh-chi.sh root@chi2810 "docker exec -e HIP_VISIBLE_DEVICES=$G mlperf_gptoss bash -lc 'cd /workspace/code/Primus-Turbo-tensorwise && /opt/venv-tw/bin/python -u <script>'"`
...
- **生产节点/环境**:
  - 节点 `chi2810` / `chi2811`,容器 `mlperf_gptoss`(gfx950 / MI355X)。
